# Fast-Check Command Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a `/fast-check` command that skips classification and uses source inspection for all claims, producing the same report format as `/check` but faster and cheaper.

**Architecture:** Same pipeline as `/check` but with classification removed and verification simplified to source-investigator only.

**Tech Stack:** Markdown (command file), JSON (plugin.json)

---

### Task 1: Create the /fast-check command

**Files:**
- Create: `plugins/midnight-fact-check/commands/fast-check.md`

- [ ] **Step 1: Create the command file**

Write `plugins/midnight-fact-check/commands/fast-check.md` with this exact content:
...
- No classification stage (no Step 5 from `/check`)
- Step 5 dispatches only source-investigator agents
- No references to `midnight-verify:verifier`
- Run directory prefix is `fast-run-` to distinguish from `/check` runs
- `run-metadata.json` includes `"mode": "fast-check"`

- [ ] **Step 3: Commit**

```bash
git add plugins/midnight-fact-check/commands/fast-check.md
git commit -m "feat(midnight-fact-check): add /fast-check command for source-only fact-checking

Streamlined pipeline: extract → verify via source inspection → report.
Skips domain classification and execution-based verification. All
claims go to source-investigator for faster, cheaper verification."
```
...
**Files:**
- Modify: `plugins/midnight-fact-check/.claude-plugin/plugin.json`
...
```bash
git add plugins/midnight-fact-check/.claude-plugin/plugin.json
git commit -m "chore(midnight-fact-check): bump version to 0.2.0"
```
